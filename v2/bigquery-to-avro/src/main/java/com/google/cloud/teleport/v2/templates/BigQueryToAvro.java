...
/**
 * The {@link BigQueryToAvro} pipeline exports data from a BigQuery table to Avro file(s) in a
 ...
 * JOB_NAME="bigquery-to-avro-`date +%Y%m%d-%H%M%S-%N`"
 *
...
public class BigQueryToAvro {
  ...
  /* Logger for class. */
  private static final Logger LOG = LoggerFactory.getLogger(BigQueryToAvro.class);
  ...
  /**
   * The {@link BigQueryToAvroOptions} class provides the custom execution options passed by the
  ...
  /**
   * The {@link BigQueryToAvro#getTableSchema(ReadSession)} method gets Avro schema for table
  ...
  public static void main(String[] args) {
    BigQueryToAvroOptions options =
        PipelineOptionsFactory.fromArgs(args).withValidation().as(BigQueryToAvroOptions.class);
   ...
   */
  private static PipelineResult run(BigQueryToAvroOptions options) {
     ...
     *        2) Write records to Google Cloud Storage in Avro format.
     */
    pipeline
        /*
        ...
        /*
         * Step 2: Write records to Google Cloud Storage as one or more Avro files
         *         via {@link AvroIO}.
         */
        .apply(
            "WriteToAvro",
